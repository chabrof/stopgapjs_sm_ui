...
/// <reference path='./i_link_registration.d.ts' />

import Tools from "./tools"
declare let document :any

let Mustache
let EltPrototype = <ILinkRegistration>Object.create(HTMLElement.prototype)

EltPrototype._approximateBezierTime = function (mousePos :IDot) {
  let mouseToLeftDotLength = Tools.getDistanceBwDots(mousePos, this._dots[0])
  let mouseToRightDotLength = Tools.getDistanceBwDots(mousePos, this._dots[1])
  return mouseToLeftDotLength / (mouseToLeftDotLength + mouseToRightDotLength)
}

EltPrototype.displayDotMoveIndics = function(dotIdx :number/*, evt*/) {
  this._setVisibilityOfHandyDots(dotIdx, true)
  this._setVisibilityOfControlDot(false)
}

EltPrototype.hideDotMoveIndics = function(dotIdx :number/*, *evt*/) {
...
EltPrototype._ctrlMove = function(evt) {
  let bezierTime = this._approximateBezierTime({ x : evt.offsetX, y : evt.offsetY })
...
EltPrototype._dotMove = function(dotIdx :number, evt) {
  let potentialDot :IDot = <IDot>{}
...
EltPrototype._computePosWGrMap = function(dotIdx :number, potentialDot :IDot) :IDot {
  let newDot :IDot = (<IGravityMap>this._gravitMovingDots[dotIdx]).computePos(potentialDot)

  return newDot
}

EltPrototype._computePosWoGrMap = function(dotIdx :number, potentialDot :IDot) :IDot {
  return potentialDot
}
...
EltPrototype._computeLength = function(dot1 :IDot, dot2 :IDot) :number {
  let a = dot1.x - dot2.x
  let b = dot1.y - dot2.y
